...
class DXF_Controller:
    ...
    def draw_openings_from_json(self, final_plan_segments, rotation_deg):
        """
        Reads door and window data from the JSON, converts their center points 
        to the DXF (mm) coordinate system, applies the global rotation/flip, 
        and draws them as 2D rectangles on the wall centerline.
        
        INCLUDES VISUAL MARKERS (CIRCLE/TEXT) for validation of the center point.
        """
        
        # 1. Load data and extract walls, openings
        with open(self.fp_json, "r") as f: # Use self.fp_json
            data = json.load(f)

        MM = 1000.0 
        measurements = data["room_measurements"]
        openings = measurements.get("doors", []) + measurements.get("windows", [])
        
        # Map wall ID to its original start/end points (in mm, X/Z swapped)
        original_walls_mm = {}
        for wall in measurements.get("walls", []):
            # Map JSON (x, y, z) to DXF (X, Y) as (x*MM, z*MM)
            start_pt = Vec2(wall["start"][0] * MM, wall["start"][2] * MM)
            end_pt = Vec2(wall["end"][0] * MM, wall["end"][2] * MM)
            original_walls_mm[wall["id"]] = {"start": start_pt, "end": end_pt}

        # 2. Get Centroid (needed for rotation/flip transform)
        all_x = [seg[0] for seg in final_plan_segments] + [seg[2] for seg in final_plan_segments]
        all_y = [seg[1] for seg in final_plan_segments] + [seg[3] for seg in final_plan_segments]
        if not all_x: return
        ...
        # 3. Define Layers
        if "OPENINGS_DOORS" not in self.doc.layers:
            self.doc.layers.add("OPENINGS_DOORS", color=2) # Yellow
        if "OPENINGS_WINDOWS" not in self.doc.layers:
            self.doc.layers.add("OPENINGS_WINDOWS", color=4) # Cyan
        
        # --- NEW DEBUG LAYER ---
        debug_layer = "DEBUG_OPENING_CENTERS"
        if debug_layer not in self.doc.layers:
            self.doc.layers.add(debug_layer, color=1) # Red

        # Constants for drawing (in mm)
        THICKNESS_MM = 100.0 # 0.1m * 1000mm/m
        DEBUG_CIRCLE_RADIUS = 50.0 
        
        # --- NEW HELPER FUNCTION TO APPLY ROTATION/FLIP ---
        def apply_transform(x, y, rotation_deg, cx, cy):
            """Applies rotation, then horizontal flip."""
            # A. Apply Rotation (Part 1 of the layout.py transform)
            th = math.radians(rotation_deg)
            ct, st = math.cos(th), math.sin(th)
            dx_raw, dy_raw = x - cx, y - cy
            rot_x = dx_raw * ct - dy_raw * st + cx
            rot_y = dx_raw * st + dy_raw * ct + cy
            # B. Apply Horizontal Flip (Part 2 of the layout.py transform)
            final_x_wcs = (2 * cx) - rot_x # Mirror across centroid X
            final_y_wcs = rot_y
            return final_x_wcs, final_y_wcs
        # --- END NEW HELPER ---
        
        for opening in openings:
            wall_id = opening.get("wallId")
            wall_data = original_walls_mm.get(wall_id)
            
            if not wall_data:
                print(f"Warning: Could not find original wall data for opening ID {opening.get('id')}. Skipping.")
                continue

            # Get opening properties (in meters -> convert to mm)
            opening_width_mm = opening["width"] * MM
            # JSON position is WCS of the opening center (x, z -> X, Y)
            raw_x_wcs = opening["position"][0] * MM
            raw_y_wcs = opening["position"][2] * MM
            
            # --- 5. Apply full transform to the OPENING'S CENTER ---
            final_x_wcs, final_y_wcs = apply_transform(raw_x_wcs, raw_y_wcs, rotation_deg, cx, cy)

            # --- 6. Apply full transform to the WALL'S ENDPOINTS ---
            wx1, wy1 = wall_data["start"].x, wall_data["start"].y
            wx2, wy2 = wall_data["end"].x, wall_data["end"].y
            
            final_wx1, final_wy1 = apply_transform(wx1, wy1, rotation_deg, cx, cy)
            final_wx2, final_wy2 = apply_transform(wx2, wy2, rotation_deg, cx, cy)
            
            # --- 7. Calculate Wall Vector from Transformed Wall Endpoints ---
            
            # We now have the correct, transformed segment corresponding to the wallId
            wall_seg = LineString([(final_wx1, final_wy1), (final_wx2, final_wy2)])
            wall_vector = Vec2(final_wx2 - final_wx1, final_wy2 - final_wy1)
            
            # Final sanity check: is the opening center actually on the transformed wall?
            if wall_seg.distance(Point(final_x_wcs, final_y_wcs)) > 1.0: # 1mm tolerance
                print(f"Warning: Opening center is too far from its calculated transformed wall for {opening.get('id')}. Skipping.")
                continue
                
            # --- Draw DEBUG Markers at the Calculated Center Point ---
            center = Vec2(final_x_wcs, final_y_wcs)
            
            # 1. Draw a small circle at the center
            self.msp.add_circle(
                center=center, 
                radius=DEBUG_CIRCLE_RADIUS, 
                dxfattribs={"layer": debug_layer}
            )
            
            # 2. Draw the coordinates for precise validation
            self.msp.add_mtext(
                f"ID: {opening['id']}\nCENTER: ({final_x_wcs:.0f}, {final_y_wcs:.0f})",
                dxfattribs={
                    'char_height': 150,
                    'insert': center + Vec2(100, 100), # Offset slightly from the center dot
                    'layer': debug_layer
                }
            )
            # --- END DEBUG MARKERS ---
            ...
            v_along = wall_vector.normalize() * (opening_width_mm / 2)
            v_perp = wall_vector.orthogonal().normalize() * (THICKNESS_MM / 2)
            
            p1 = center - v_along + v_perp
            p2 = center + v_along + v_perp
            p3 = center + v_along - v_perp
            p4 = center - v_along - v_perp
            
            points = [p1, p2, p3, p4]
            
            layer = "OPENINGS_DOORS" if "door" in opening["id"] else "OPENINGS_WINDOWS"
            ...
            self.msp.add_lwpolyline( 
                ...
                dxfattribs={
                    "layer": layer,
                    "color": 256 # ByLayer default color
                }
            )

            # Optional: Add a debug text label (opening ID)
            self.msp.add_mtext( 
                opening["id"],
                dxfattribs={
                    'char_height': 150,
                    'insert': center - Vec2(200, 200), # Place slightly below/left
                    'layer': layer
                }
            )
                
        print("✅ Finished drawing doors and windows in DXF.")
...
#----normal--place-clinic--function-----------------------------------------------------------
    ...
    def _find_best_combination_for_row_perm(self, clinic_names_to_place: list, available_width: float, force_orientation: Optional[str] = None, start_vertical_count: int = 0) -> List[dict]:
        """
        Try layouts using permutations (optional with safety cap) and all orientation combos.
        Keeps start_vertical_count to continue the V-V alternating gap pattern across rows.
        """
        ...
        names = list(clinic_names_to_place)
        # Safety: avoid factorial explosion - limit permutations if many clinics remain
        MAX_PERMUTE = 6
        use_permutations = len(names) <= MAX_PERMUTE

        if not use_permutations and len(names) > MAX_PERMUTE:
            # fallback: test prefixes only (greedy) to keep runtime reasonable
            for n in range(len(names), 0, -1):
                try:
                    fixtures = [Fixture.Fixture(self.fixture_dict[name]["name"], self.fixture_dict[name]["path"]) for name in names[:n]]
                except Exception:
                    continue
                for combo in product(orientations_to_test, repeat=n):
                    required_width = 0.0
                    gaps = []
                    ...
                    gaps.append(0)
                    total_required_space = 1.0 + required_width + 1.0
                    if total_required_space <= available_width:
                        score = (n * 10000) - required_width
                        if 'H' in combo: score += 500
                        if 'V' in combo and 'H' in combo: score += 200
                        if combo.count('V') == 0 and len(combo) > 1: score += 1000
                        layout = {
                            'score': score,
                            'combo': combo,
                            'gaps': gaps,
                            'fixtures_for_row': fixtures,
                            'start_margin': 1.0,
                            'remaining_fixtures_names': names[n:]
                        }
                        layout['Placed'] = False
                        valid_layouts.append(layout)
            valid_layouts.sort(key=lambda x: x['score'], reverse=True)
            # --- Print top 10 layouts for debugging / inspection (fallback branch) ---
            import json
            def _ser_layout(l):
                return {
                    "score": l.get("score"),
                    "combo": "".join(l.get("combo", [])) if isinstance(l.get("combo"), (list, tuple)) else l.get("combo"),
                    "gaps": l.get("gaps"),
                    "fixtures": [getattr(f, "name", str(f)) for f in l.get("fixtures_for_row", [])]
                }
            top10 = valid_layouts[:10]
            print("Top 10 layouts (fallback):")
            print(json.dumps([_ser_layout(l) for l in top10], indent=2))

            return valid_layouts

        # Permutation mode (safe because of MAX_PERMUTE)
        for k in range(len(names), 0, -1):
            for order in permutations(names, k):
                try:
                    fixtures = [Fixture.Fixture(self.fixture_dict[nm]["name"], self.fixture_dict[nm]["path"]) for nm in order]
                except Exception:
                    continue
                for combo in product(orientations_to_test, repeat=k):
                    required_width = 0.0
                    gaps = []
                    total_vertical_clinics = int(start_vertical_count)
                    for i in range(k):
                    ...
                    if total_required_space <= available_width:
                        ...
                        layout = {
                            ...
                            'fixtures_for_row': fixtures,
                            'start_margin': 1.0,
                            'remaining_fixtures_names': [n for n in names if n not in order]
                        }
                        layout['Placed'] = False
                        valid_layouts.append(layout)
        ...
        if valid_layouts:
            ...
            # === DEDUPE: keep first occurrence of (combo, multiset_of_fixture_names) ===
            seen = set()
            deduped = []
            for l in valid_layouts:
                # normalize key: orientation combo + sorted fixture names (multiset)
                combo_key = tuple(l.get('combo', ()))
                fixture_names = tuple(sorted(getattr(f, "name", str(f)) for f in l.get('fixtures_for_row', [])))
                key = (combo_key, fixture_names)
                if key in seen:
                    continue
                seen.add(key)
                deduped.append(l)

            valid_layouts = deduped
            ...
            print(f"  -> _find_best_combination_for_row: returning {len(valid_layouts)} unique layouts, showing top {len(top10)}")
            print(json.dumps([_ser_layout(l) for l in top10], indent=2))
    ...
    def _find_best_combination_for_row(self, clinic_names_to_place: list, available_width: float, force_orientation: Optional[str] = None, start_vertical_count: int = 0) -> List[dict]:
        ...
        for n in range(len(clinic_names_to_place), 0, -1):
            ...
            for combo in orientations:
                ...
                start_margin = 1.0 if combo[0] == 'V' else 1.0
                end_margin = 1.0 if combo[-1] == 'V' else 1.0
    ...
    def _calculate_opportunistic_boh_zone_v1(self, last_clinic_bbox: tuple, placed_bboxes: List[tuple], last_clinic_orientation: str, big_partition_threshold_mm: float = 800.0) -> Tuple[Optional[Polygon], float]:
        """
    ...
    def _calculate_opportunistic_boh_zone(self, last_clinic_bbox: tuple, placed_bboxes: List[tuple], last_clinic_orientation: str, vertical_clinic_count: int = 1, big_partition_threshold_mm: float = 800.0) -> Tuple[Optional[Polygon], float]:
        """
        [ROBUST VERSION 3 - PARTITION AWARE] Calculates the BOH zone by subtracting the retail area
        from the entire floorplan, AND now stops at the first major internal partition.
        """
        ...
        # Alternating gap logic for vertical clinics
        start_x_for_boh = lc_max_x
        if last_clinic_orientation == 'V':
            if vertical_clinic_count % 2 == 1:
                print("    -> (Verification) Applying 800mm gap for odd vertical clinic.")
                start_x_for_boh += 800.0
            else:
                print("    -> (Verification) No gap for even vertical clinic.")
                # start_x_for_boh += 0.0
    ...
    def _find_and_place_row_in_zone(self, zone, layouts_ranked: list, clinics_queue, placed_bboxes, placed_clinics_on_top_wall,start_vertical_count: int = 0):
        ...
        while search_cursor < zone['length'] - 50.0:
            ...
            for layout in layouts_ranked:
                #  *************************************************
                if layout.get("Placed", False):  # Skip already placed layouts
                    continue
                #  *************************************************
                ...
                if is_entire_row_valid:
                    ...
                    # ***************************************************
                    layout["Placed"] = True # Mark this layout as used
                    # ***************************************************
    ...
    # best_layouts_ranked_cache = None  # Cache for best_layouts_ranked
    def reset_best_layouts_cache(self):
        self.best_layouts_ranked_cache = None
    ...
    def place_clinics_master_strategy(self, placed_bboxes: List[tuple]):
        ...
        for zone in top_wall_zones:
            ...
            #*****************************************************
            # # Compute best_layouts_ranked only once
            # if self.best_layouts_ranked_cache is None:
            #     self.best_layouts_ranked_cache = self._find_best_combination_for_row(
            #         clinic_names_for_solver, available_width, force_orientation=None, start_vertical_count=total_vertical_clinics_placed_on_top
            #     )
            #     print("  -> Cached best_layouts_ranked for reuse.")           
            # Compute best_layouts_ranked only once
            if self.best_layouts_ranked_cache is None:
                print("  -> Calling _find_best_combination_for_row to compute layouts (cache miss).")
                self.best_layouts_ranked_cache = self._find_best_combination_for_row(
                    clinic_names_for_solver, available_width, force_orientation=None, start_vertical_count=total_vertical_clinics_placed_on_top
                )
                if self.best_layouts_ranked_cache:
                    print(f"  -> Cached {len(self.best_layouts_ranked_cache)} layouts (best score: {self.best_layouts_ranked_cache[0]['score']})")
                else:
                    print("  -> Cache created but no layouts returned.")
            else:
                print("  -> Using cached best_layouts_ranked (cache hit).")
            #*****************************************************

            # best_layouts_ranked = self._find_best_combination_for_row(clinic_names_for_solver, available_width,force_orientation=None)

            # # ********************************************************************
            # best_layouts_ranked = self.best_layouts_ranked_cache
            # Priortize_Orientation = "H"  # Can be "H", "V", or None
            # if Priortize_Orientation in ["H", "V"]:
            #     # Sort the layouts based on the count of the prioritized orientation
            #     best_layouts_ranked.sort(
            #         key=lambda layout: -layout['combo'].count(Priortize_Orientation)
            #     )

            # #******************************************************
            best_layouts_ranked = self.best_layouts_ranked_cache

            # --- NEW: prioritize explicit full-combo patterns in a fixed order ---
            Priortize_list = []  # order = highest priority first
            if isinstance(best_layouts_ranked, list) and Priortize_list:
                # map pattern -> priority index
                priority_index = {p: i for i, p in enumerate(Priortize_list)}

                # stable sort: prioritized combos get their priority index, others get +inf
                def _priority_key(layout):
                    combo = layout.get('combo')
                    combo_str = "".join(combo) if isinstance(combo, (list, tuple)) else str(combo)
                    return priority_index.get(combo_str, float('inf'))

                best_layouts_ranked.sort(key=_priority_key)

                # report whether any prioritized patterns were present
                prioritized_found = [l for l in best_layouts_ranked if "".join(l.get('combo','')) in Priortize_list]
                if prioritized_found:
                    print(f"    -> Prioritized patterns present: {[ ''.join(l.get('combo')) for l in prioritized_found[:len(Priortize_list)] ]}")
                else:
                    print("    -> Prioritized patterns not found: None")

            
            # --- NEW: Print top 10 layouts for this zone so you can inspect them ---
            import json
            def _ser_layout(l):
                return {
                    "score": l.get("score"),
                    ...
                    "fixtures": [getattr(f, "name", str(f)) for f in l.get("fixtures_for_row", [])],
                    "gaps": l.get("gaps"),
                    "Placed": l.get("Placed", False)
                }
            top10 = best_layouts_ranked[:10]
            print(f"    -> TOP {len(top10)} layouts for zone length {available_width:.0f}mm:")
            print(json.dumps([_ser_layout(l) for l in top10], indent=2))
            ...
            #     def serialize_fixture(fxtr):
            #         return getattr(fxtr, 'name', str(fxtr))
            #     out = []
            #     for layout in layouts:
            #         d = dict(layout)
            #         # Replace Fixture objects with their names
            #         d['fixtures_for_row'] = [serialize_fixture(f) for f in d['fixtures_for_row']]
            #         out.append(d)
            #     return out
            # top10 = best_layouts_ranked[:10]
            # print("best layouts ranked before placement:", json.dumps(serialize_layouts(best_layouts_ranked), indent=4))
            # print("best layouts ranked before placement:",best_layouts_ranked)
            #***********************************************************************************
            ...
            # print("best layouts ranked after placement:", json.dumps(serialize_layouts(best_layouts_ranked), indent=1))
            # print("best layouts ranked after placement:", best_layouts_ranked)
            ...
            top10 = best_layouts_ranked[:10]
            print(f"    -> TOP {len(top10)} After placement {available_width:.0f}mm:")
            print(json.dumps([_ser_layout(l) for l in top10], indent=2))
        ...
        if not placed_clinics_on_top_wall:
            ...
            boh_zone_poly, boh_area_sqft = self._calculate_opportunistic_boh_zone(
                ...
                vertical_clinic_count=total_vertical_clinics_placed_on_top
            )
            # --- END OF MODIFICATION ---
            print(f"    -> Final check: Potential BOH area is {boh_area_sqft:.2f} sq. ft.")
            
            # if you need to visualize the boh zone during debugging, uncomment below
             # Draw the BOH zone for debugging purposes
            # if boh_zone_poly and not boh_zone_poly.is_empty:
            #     layer_name = "DEBUG_BOH_ZONE"
            #     if layer_name not in self.doc.layers:
            #         self.doc.layers.add(name=layer_name, color=3)  # Green for visibility
            #     zone_boundary_coords = list(boh_zone_poly.exterior.coords)
            #     self.msp.add_lwpolyline(zone_boundary_coords, close=True, dxfattribs={"layer": layer_name, "lineweight": 50})
                # print(f"✅ BOH zone drawn for visualization. Area: {area_in_sqft:.2f} sq.ft.")
    ...
    def place_clinic_under_first_row(self, clinics_queue: collections.deque, placed_bboxes: list, placed_clinics_on_top_wall: list):
        ...
        for zone in placement_zones:
            ...
            # layouts_ranked = self._find_best_combination_for_row(clinic_names_for_solver, zone['length'], force_orientation=None)
            
            print(f"      -> Found {len(layouts_ranked)} possible layouts for zone '{zone['id']}'.")
            import json
            def serialize_layouts(layouts):
                def serialize_fixture(fxtr):
                    return getattr(fxtr, 'name', str(fxtr))
                ...
                for layout in layouts:
                    ...
                    out.append(d)
                return out
            # print("best layouts ranked :", json.dumps(serialize_layouts(layouts_ranked), indent=1))
            # print(f"      -> Layouts: {layouts_ranked}")
            if not layouts_ranked:
            ...
            for i, fxtr in enumerate(best_layout['fixtures_for_row']):
                ...
                extra_offset = 100
                target_center_y = y_pos - height / 2 + extra_offset
...
# (This function can be placed where the old one was)
    ...
    def place_clinics_master_strategy_opposite(self, placed_bboxes: List[tuple]):
        ...
        top_wall_zones = self._analyze_top_wall_with_bulge_detection_opposite(small_bulge_max_width=1000, 
            small_bulge_max_depth=1000
        )
        ...
        for zone in top_wall_zones:
            ...
            top10 = best_layouts_ranked[:10]
            print(f"    -> TOP {len(top10)} After layouts for zone length {available_width:.0f}mm:")
            print(json.dumps([_ser_layout(l) for l in top10], indent=2))
...
#---------------------new--setup-for-findong-area-for-euro-center-placement--
    ...
    def generate_row_wise_grid(self) -> List[Tuple[float, float]]:
        """
        [REFINED v2] Calculates grid for 0° rotation with adjusted thresholds and added debugging.
        Rules: >= 90% in zone, NO partition overlap, <= 70% aisle blockage.
        """
        from shapely.geometry import box, Point
        from shapely.ops import unary_union
        import math

        print("\n--- MATRIX Generating ROW-WISE grid (0° Rotation) [v2 - Debugging] ---")
        zone = self.euro_center_placement_area()
        if not zone or zone.is_empty:
            print("  -> SKIPPED: Valid placement zone is empty or undefined.")
            return []

        # --- Get Obstacles and Partitions ONCE (Same as before) ---
        all_obstacle_bboxes = self._get_accurate_obstacle_bboxes(include_all=True)
        separator_line = self.msp.query('LINE[layer=="RETAIL_SEPARATOR"]').first
        if separator_line:
            ...
            all_obstacle_bboxes = [b for b in all_obstacle_bboxes if b != separator_bbox]
        ...
        fixed_obstacles_union = unary_union(fixed_obstacles_for_aisle_check) if fixed_obstacles_for_aisle_check else None

        # --- Grid Parameters (ADJUSTED THRESHOLDS) ---
        cell_width = 1040.0
        cell_height = 1175.0
        aisle_width = 1050.0
        min_containment_ratio = 0.70 # Increasing the value making stricter and decreasing the value making looser
        max_aisle_blockage_ratio = 0.30 # Increasing the value making looser and decreasing the value making stricter

        final_grid_points = []
        min_x_zone, min_y_zone, max_x_zone, max_y_zone = zone.bounds

        print(f"  -> Zone Bounds: ({min_x_zone:.0f}, {min_y_zone:.0f}) to ({max_x_zone:.0f}, {max_y_zone:.0f})")
        print(f"  -> Applying Rules: Containment >= {min_containment_ratio*100}%, Aisle Blockage <= {max_aisle_blockage_ratio*100}%")
        ...
        while current_y + cell_height <= max_y_zone:
            ...
            while current_x + cell_width <= max_x_zone:
                ...
                reason = "OK" # For debugging output

                # --- Rule 1: Partial Zone Containment ---
                containment_ratio = 0.0
                if cell_area > 0:
                    intersection_area = zone.intersection(cell_box).area
                    containment_ratio = intersection_area / cell_area
                
                if containment_ratio < min_containment_ratio:
                    reason = f"FAIL Rule 1 (Containment: {containment_ratio:.2f} < {min_containment_ratio:.2f})"
                    # DEBUG: print(f"    - Cell ({current_x:.0f},{current_y:.0f}): {reason}") # Uncomment if needed
                    current_x += cell_width
                    continue

                # --- Rule 2: No Partition Overlap ---
                if any(cell_box.intersects(partition_poly) for partition_poly in partition_polygons):
                    reason = "FAIL Rule 2 (Partition Overlap)"
                    # DEBUG: print(f"    - Cell ({current_x:.0f},{current_y:.0f}): {reason}") # Uncomment if needed
                    current_x += cell_width
                    continue

                # --- Rule 3: Aisle Clearance ---
                ...
                for aisle_name, aisle_box in [("Top", top_aisle), ("Bottom", bottom_aisle)]:
                    aisle_area = aisle_box.area
                    if aisle_area == 0: continue

                    overlap_area = 0
                    if fixed_obstacles_union and aisle_box.intersects(fixed_obstacles_union):
                        overlap_area = aisle_box.intersection(fixed_obstacles_union).area

                    blockage_ratio = overlap_area / aisle_area if aisle_area > 0 else 0

                    if blockage_ratio > max_aisle_blockage_ratio:
                        reason = f"FAIL Rule 3 ({aisle_name} Aisle Blockage: {blockage_ratio:.2f} > {max_aisle_blockage_ratio:.2f})"
                        # DEBUG: print(f"    - Cell ({current_x:.0f},{current_y:.0f}): {reason}") # Uncomment if needed
                        aisles_valid = False
                        break

                # --- Add Point if All Rules Pass ---
                if aisles_valid:
                    final_grid_points.append((current_x, current_y))
                    # DEBUG: print(f"    - Cell ({current_x:.0f},{current_y:.0f}): PASS") # Uncomment if needed
                
                current_x += cell_width
            current_y += cell_height

        print(f"  -> ✅ Generated {len(final_grid_points)} valid spots for row-wise placement.")
        return final_grid_points

    def generate_column_wise_grid(self) -> List[Tuple[float, float]]:
        """
        [REFINED v2] Calculates grid for 90° rotation with adjusted thresholds and added debugging.
        Rules: >= 90% in zone, NO partition overlap, <= 70% aisle blockage.
        """
        from shapely.geometry import box, Point
        from shapely.ops import unary_union
        ...
        print("\n--- MATRIX Generating COLUMN-WISE grid (90° Rotation) [v2 - Debugging] ---")
        zone = self.euro_center_placement_area()
        if not zone or zone.is_empty:
            print("  -> SKIPPED: Valid placement zone is empty or undefined.")
            return []
        ...
        # --- Grid Parameters (Rotated, ADJUSTED THRESHOLDS) ---
        cell_width = 1175.0
        cell_height = 1040.0
        aisle_width = 1050.0
        min_containment_ratio = 0.70 # Increasing the value making stricter and decreasing the value making looser
        ...
        while current_y + cell_height <= max_y_zone:
            ...
            while current_x + cell_width <= max_x_zone:
                ...
                for aisle_name, aisle_box in [("Left", left_aisle), ("Right", right_aisle)]:
                    aisle_area = aisle_box.area
                    if aisle_area == 0: continue
        ...
        print(f"  -> ✅ Generated {len(final_grid_points)} valid spots for column-wise placement.")
        return final_grid_points

    # ... (rest of DXF_Controller methods) ...
    ...
    def arranging_analyzed_function(self, display_calcs: dict, analysis_result: dict) -> dict:
        """
        ...
        current_analysis_options = json.loads(json.dumps(analysis_result.get('all_options', {})))

        required_count = display_calcs.get('floor_fixtures', 0)
        overflow_display = 0
        ...
        # --- Find the highest-ranked pattern that hasn't been placed ---
        available_patterns = [
            (name, data) for name, data in current_analysis_options.items()
            if not data.get('placed', False)
        ]

        if not available_patterns:
            print("  -> ⚠️ No available patterns left to choose from.")
            return {
                'chosen_pattern_name': "None",
                'placements': {},
                'overflow_display': required_count
            }

        available_patterns.sort(key=lambda item: item[1].get('rank', float('inf')))
    ...
    #-----------------------------------------------------------------------
    # Adjust place_by_plan_euro
    #-----------------------------------------------------------------------

    # def place_by_plan_euro(self, arranged_plan: dict, placed_bboxes: list): # Original signature
    def place_by_plan_euro(self, arranged_plan: dict, analysis_result: dict, placed_bboxes: list):
        """
        [MODIFIED] Places Euro Centre fixtures based *only* on the coordinates provided in the
        arranged plan. It now updates the 'placed' status in the main analysis_result
        after successful placement.

        Args:
            arranged_plan: The output dictionary from arranging_analyzed_function().
            analysis_result: The original full analysis dictionary to update the 'placed' status.
            placed_bboxes: The master list of bounding boxes to be updated.
        """
        ...
        if chosen_pattern_name == "None" or not placements_to_execute:
            print("  -> SKIPPED: No valid placement plan provided in arranged_plan.")
            # Make sure the original analysis_result reflects no placement if none happened
            if chosen_pattern_name != "None" and chosen_pattern_name in analysis_result['all_options']:
                analysis_result['all_options'][chosen_pattern_name]['placed'] = False # Ensure it's false if we skip
            return # Exit the function

        print(f"  -> Executing plan for pattern: '{chosen_pattern_name}'")
        print(f"  -> Number of fixtures to place: {len(placements_to_execute)}")

        # --- Determine Rotation and Load Fixture ---
        try:
            fxtr = Fixture.Fixture("Euro_centre", self.fixture_dict["Euro_centre"]["path"])
        except Exception as e:
            print(f"  -> 🔥 FATAL: Could not load Euro_centre fixture: {e}")
            # Mark as not placed if fixture loading fails
            if chosen_pattern_name in analysis_result['all_options']:
                analysis_result['all_options'][chosen_pattern_name]['placed'] = False
            return
        ...
        # --- Placement Loop ---
        placed_count = 0
        # Sort keys numerically (count_1, count_2, etc.) for predictable order
        ...
        for key in sorted_placement_keys:
            ...
            if not coords or len(coords) != 2:
                print(f"    -> ⚠️ Skipping {key}: Invalid coordinates.")
                continue
            ...
            try:
                ...
                block_ref = self.place_fixture(
                    ...
                    (final_insert_point.x, final_insert_point.y, 0),
                    rotation,
                    # rotated=True # Keep as True assuming it aligns fixtures correctly to walls/grids
                    rotated = is_rotated # Use the flag directly
                )
                ...
                if block_ref:
                    ...
                    if bbox.has_data:
                        ...
                        placed_bboxes.append(new_bbox_tuple)
                        print(f"    -> Placed Euro_centre {key} at ({x:.0f}, {y:.0f}), Rotation: {rotation}°, Added BBox.")
                        placed_count += 1
                    else:
                        print(f"    -> Placed Euro_centre {key} but could not calculate bounding box.")
                else:
                    print(f"    -> ⚠️ Failed to place Euro_centre {key}.")

            except Exception as e:
                print(f"    -> ⚠️ Error placing Euro_centre {key}: {e}")
                traceback.print_exc()
                # If one fails, we might still continue, but won't mark pattern as placed yet

        # --- Update Placed Status ---
        # Only mark as placed if at least one fixture was successfully placed using this plan
        if placed_count > 0:
            if chosen_pattern_name in analysis_result.get('all_options', {}):
                analysis_result['all_options'][chosen_pattern_name]['placed'] = True
                print(f"  -> Marked pattern '{chosen_pattern_name}' as placed in analysis_result.")
            else:
                print(f"  -> ⚠️ Could not find pattern '{chosen_pattern_name}' in analysis_result to mark as placed.")
        else:
            # Ensure it's marked false if zero placements occurred for this pattern
            if chosen_pattern_name in analysis_result.get('all_options', {}):
                analysis_result['all_options'][chosen_pattern_name]['placed'] = False

        print(f"\n--- ✅ Finished Direct Placement: Placed {placed_count} Euro Centres based on the plan. ---")
...
#-----------------------new  for wall_fixture placement---------------------------------------------------------------------------------------
    ...
    best_analyze_ranked_cache = None

    def reset_best_analyze_ranked_cache(self):
        """ 
        Resets the cached analysis for Euro Center placement patterns.
        """
        self.best_analyze_ranked_cache = None
        print("  -> Cache for best_analyze_ranked_cache has been reset.")
    ...
    def plan_and_place_euro_fixtures(self, all_placed_bboxes: list, remaining_wall_fixtures: int, display_calcs: dict, draw_debug: bool = False):
        ...
        # 3. Use cached analysis or perform analysis
        if self.best_analyze_ranked_cache is None:
            print("\n--- Performing Euro Center Placement Analysis ---")
            self.best_analyze_ranked_cache = self.analyze_placement_patterns()
        else:
            print("\n--- Using Cached Euro Center Placement Analysis ---")

        # placement_blueprint = self.analyze_placement_patterns()
        placement_blueprint = self.best_analyze_ranked_cache
        
        final_plan_details = self.arranging_analyzed_function(display_calcs, placement_blueprint)
        print("\nBefore placement_blueprint with rank and placed:")
        print(json.dumps(placement_blueprint, indent=2))        
        #**************************************************************
        
        # Store overflow count from the arrangement phase
        self.overflow_fixture_count = final_plan_details.get('overflow_display', 0)
        print(f"  -> Overflow fixtures calculated during arrangement: {self.overflow_fixture_count}")

        # 5. Place based on the arranged plan
        # ***MODIFICATION: Pass the main analysis_result_to_update dictionary***
        self.place_by_plan_euro(final_plan_details, placement_blueprint, all_placed_bboxes)

        # Optional: Print the updated status for verification
        # if draw_debug:
        print("\n--- Analysis Result after Placement Update ---")
        print(json.dumps(placement_blueprint, indent=2))

        print("--- ✅ Euro Center Placement Orchestration Complete ---")
        # # print("\n placement dictionary", json.dumps(placement_blueprint, indent=2))
        # self.place_by_plan_euro(final_plan_details, all_placed_bboxes)
        # # self.place_euro_centers_from_blueprint(placement_blueprint, display_calcs, all_placed_bboxes)
        
        # # Mark the best pattern as placed
        # best_pattern_name = placement_blueprint.get("best_pattern_name")
        # all_options = placement_blueprint.get("all_options", {})
        # if best_pattern_name in all_options:
        #     all_options[best_pattern_name]["placed"] = True
        # print("\nAfter placement_blueprint with rank and placed:")
        # print(json.dumps(placement_blueprint, indent=2))    
